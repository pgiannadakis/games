...
def rk4_step(bodies: List[Body], dt: float) -> None:
    ...
    def make_state(ps, vs):
        return [
            Body(
                b.name, b.mass, ps[i].copy(), vs[i].copy(),
                b.color, b.radius, b.affects_others
            )
...
def make_buttons() -> List[Button]:
    ...
    for i, (label, action) in enumerate(labels):
        col = i % 2
        row = i // 2
        rect = pygame.Rect(x + col * (w + 16), y + row * (h + gap), w, h)
...
def make_param_controls(start_y: int):
    ...
    for i, field in enumerate(fields):
        y = start_y + i * row_h
        minus_rect = pygame.Rect(minus_x, y, 36, 30)
        plus_rect = pygame.Rect(plus_x, y, 36, 30)
...
def draw_bodies(screen, bodies, camera_center, zoom, font, selected_idx):
    for i, b in enumerate(bodies):
        ...
        screen.blit(label, (sx + b.radius + 4, sy - b.radius - 4))
...
def draw_sidebar(screen, font, small_font, buttons, selected_body, mode, paused, follow_com):
    ...
    draw_text(screen, font, "Three-Body Sandbox", TEXT_COLOR, SIM_WIDTH + 20, 120)
    draw_text(screen, small_font, f"Mode: {mode.upper()}", MUTED_TEXT, SIM_WIDTH + 20, 155)
    draw_text(screen, small_font, f"Paused: {paused}", MUTED_TEXT, SIM_WIDTH + 20, 180)
    draw_text(screen, small_font, f"Follow COM: {follow_com}", MUTED_TEXT, SIM_WIDTH + 20, 205)
    ...
    for button in buttons:
        ...
        pygame.draw.rect(screen, (120, 130, 170), button.rect, 2, border_radius=8)
    ...
    draw_text(screen, small_font, f"tiny mass body: {not selected_body.affects_others}", MUTED_TEXT, SIM_WIDTH + 20, y0 + 65)
    ...
    for field, label_x, y, minus_rect, plus_rect in controls:
        ...
        pygame.draw.rect(screen, BUTTON_BG, minus_rect, border_radius=6)
        pygame.draw.rect(screen, BUTTON_BG, plus_rect, border_radius=6)
        pygame.draw.rect(screen, (120, 130, 170), minus_rect, 2, border_radius=6)
        pygame.draw.rect(screen, (120, 130, 170), plus_rect, 2, border_radius=6)

        draw_text(screen, small_font, "-", BUTTON_TEXT, minus_rect.x + 12, minus_rect.y + 3)
        draw_text(screen, small_font, "+", BUTTON_TEXT, plus_rect.x + 11, plus_rect.y + 2)

    help_y = y0 + 110 + 5 * 48 + 40
    help_lines = [
        "Edit mode:",
        "  left-drag body to move",
        "  click +/- to tune parameters",
        "  mouse wheel to zoom",
        "  right-drag to pan",
        "",
        "Run mode:",
...
def handle_sidebar_click(mouse_pos, buttons, selected_body):
    for button in buttons:
    ...
    controls = make_param_controls(270 + 110)
            ...
            if event.type == pygame.QUIT:
                ...
                if event.button == 1:
                    if point_in_sim_area(event.pos):
                        ...
                        kind, key, sign = handle_sidebar_click(event.pos, buttons, selected_body)
                ...
                pos_step = 0.15 if (mods & pygame.KMOD_SHIFT) else 0.03
                vel_step = 0.15 if (mods & pygame.KMOD_SHIFT) else 0.03
                ...
                if event.key == pygame.K_SPACE:
                    ...
                    TIME_SCALE = min(20.0, TIME_SCALE * 1.2)

                elif event.key == pygame.K_DOWN and mode == "run":
                    TIME_SCALE = max(0.05, TIME_SCALE / 1.2)
